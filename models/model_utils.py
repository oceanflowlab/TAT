...
from utils.paths import WEIGHTS_PATH
...
def load_last_checkpoint(name, model, device="cuda", strict=True, remove_name_preffix=None, remove_name_postfix=None):
    weights_path = glob(os.path.join(WEIGHTS_PATH, name, f"weights-epoch=*.ckpt"))[0]
    # weights_path = glob(os.path.join(WEIGHTS_PATH, f"weights-epoch=*.ckpt"))[0]
