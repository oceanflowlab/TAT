...
FEATURE_STRIDE = int(os.environ.get("DROPD_TW_FEATURE_STRIDE", "32"))
...
class LMDB_Folder_Dataset(Dataset):
    ...
    def filter_task_ids(self, task_ids):
        task_ids = {int(task_id) for task_id in task_ids}
        self.cls_datasets = [
            dataset
            for dataset in self.cls_datasets
            if dataset.cls_id is not None and int(dataset.cls_id) in task_ids
        ]
        self.cls_lens = [len(dataset) for dataset in self.cls_datasets]
        self.cls_end_idx = np.cumsum(self.cls_lens)
...
class LMDB_Class_Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        try:
            sample_dict["steps"] = full_sample_dict["steps"]
        except:
            pass
        # get step features
        ...
        if self.cls_id is not None:
            sample_dict["step_starts"] = np.array(
                [Time2FrameNumber(s, 10) // FEATURE_STRIDE for s in full_sample_dict["steps_starts"]]
            )
            sample_dict["step_ends"] = np.array(
                [Time2FrameNumber(s, 10) // FEATURE_STRIDE for s in full_sample_dict["steps_ends"]]
            )
            last_frame_idx = max(int(sample_dict["num_frames"]) - 1, 0)
            sample_dict["step_starts"] = np.clip(sample_dict["step_starts"], 0, last_frame_idx)
            sample_dict["step_ends"] = np.clip(sample_dict["step_ends"], 0, last_frame_idx)
        else:
            sample_dict["step_starts"] = np.array([int(s) for s in full_sample_dict["steps_starts"]])
            sample_dict["step_ends"] = np.array([int(s) for s in full_sample_dict["steps_ends"]])
